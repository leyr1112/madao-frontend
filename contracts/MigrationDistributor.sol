...
contract MigrateDistributor is Policy {
    ...
    address public immutable sMadaoOld;
    
    address public newStaking;
    address public sMadaoNew;
    ...
    */
    ...
    constructor( address _treasury, address _inte, uint _epochLength, uint _nextEpochBlock, address _oldStaking, address _sMadaoOld, address _newStaking, address _sMadaoNew ) {
        require( _treasury != address(0) );
        treasury = _treasury;
        require( _inte != address(0) );
        INTE = _inte;
        require( _oldStaking != address(0) );
        oldStaking = _oldStaking;
        require( _sMadaoOld != address(0) );
        sMadaoOld = _sMadaoOld;
        require( _newStaking != address(0) );
        newStaking = _newStaking;
        require( _sMadaoNew != address(0) );
        sMadaoNew = _sMadaoNew;
    ...
    function distribute() external returns ( bool ) {
        if ( nextEpochBlock <= block.number ) {
            ...
            uint oldReward= split(reward,ISINTE(sMadaoOld).circulatingSupply(),ISINTE(sMadaoNew).circulatingSupply());
    ...
    function split( uint reward, uint supply1, uint supply2 ) public pure returns ( uint _reward1 ) {
        if(reward==0){
            return 0;
        }else{
            uint total=supply1.add(supply2);
            uint percent=supply1.mul(10000).div(total);
            uint reward1=reward.mul(percent).div(10000);
    ...
    function nextRewardAt( uint _rate ) public view returns ( uint ) {
        return IERC20( INTE ).totalSupply().mul( _rate ).div( 1000000 );
    }
    ...
    function setNewStaking(address _newStaking, address _sMadaoNew) external onlyPolicy() {
        require( _newStaking != address(0) );
        newStaking = _newStaking;
        require( _sMadaoNew != address(0) );
        sMadaoNew = _sMadaoNew;
