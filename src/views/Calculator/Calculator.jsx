...
function Calculator() {
  ...
  const trimmedsHecBalance = trim(Number(smadaoBalance), 4);
  const trimeMarketPrice = trim(marketPrice, 2);

  const [smadaoAmount, setsHecAmount] = useState(trimmedsHecBalance);
  ...
  const calcInitialInvestment = () => {
    const smadao = Number(smadaoAmount) || 0;
    const price = parseFloat(priceAtPurchase) || 0;
    const amount = smadao * price;
  ...
  }, [days, rewardYield, futureMarketPrice, smadaoAmount]);
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={6} sm={4} md={4} lg={4}>
                    <Box className="calculator-card-index">
                      ...
                      <Typography variant="h4" style={{ color: "#FFAE00" }}>
                        {isAppLoading ? <Skeleton width="100px" /> : <>{trimmedsHecBalance} sMADAO</>}
                      </Typography>
                    </Box>
                  </Grid>
                </Grid>
            ...
            <Box className="calculator-card-area">
              <Box>
                  ...
                  <Grid container spacing={3}>
                    <Grid item xs={12} sm={6}>
                      <Box className="calculator-card-action-area-inp-wrap">
                        <Typography variant="h6">sMADAO Amount</Typography>
                        <OutlinedInput
                          type="number"
                          placeholder="Amount"
                          ...
                          endAdornment={
                            <InputAdornment position="end">
                              <div
                                onClick={() => setsHecAmount(trimmedsHecBalance)}
                    ...
                    <Grid item xs={12} sm={6}>
                      <Box className="calculator-card-action-area-inp-wrap">
                        <Typography variant="h6">MADAO price at purchase ($)</Typography>
                        <OutlinedInput
                          type="number"
                          placeholder="Amount"
                          className="calculator-card-action-input"
                    ...
                    <Grid item xs={12} sm={6}>
                      <Box className="calculator-card-action-area-inp-wrap">
                        <Typography variant="h6">Future MADAO market price ($)</Typography>
                        <OutlinedInput
                          type="number"
                          placeholder="Amount"
                          className="calculator-card-action-input"
                ...
                <Box className="calculator-user-data">
                  ...
                  <Box className="data-row">
                    <Typography>MADAO rewards estimation</Typography>
                    <Typography>{isAppLoading ? <Skeleton width="80px" /> : <>{rewardsEstimation} MADAO</>}</Typography>
                  </Box>
                  <Box className="data-row">
