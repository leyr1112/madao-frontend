...
import { loadNftMintDetails } from "./slices/NftMintSlice";
import { loadAccountDetails, calculateUserBondDetails } from "./slices/AccountSlice";
import { info } from "./slices/MessagesSlice";

import { Stake, ChooseBond, Bond, TreasuryDashboard, Calculator, NftMint, NftStake } from "./views";
...
import "./style.scss";
import { madao_busd } from "./helpers/AllBonds";
...
function App() {
  ...
  const loadApp = useCallback(
    loadProvider => {
      ...
      dispatch(loadNftMintDetails({ networkID: chainID, provider: loadProvider, address }));
    },
    [connected],
  );
  ...
  useEffect(() => {
    ...
    if (shouldTriggerSafetyCheck()) {
      dispatch(info("Safety Check: Always verify you're on http://mysticaliens.com/!"));
    }
  }, []);
  ...
  useEffect(() => {
    if (connected) {
      const updateAppDetailsInterval = setInterval(() => {
        ...
        dispatch(loadNftMintDetails({ networkID: chainID, provider, networkID: chainID, address }));
      }, 1000 * 60);
      return () => {
